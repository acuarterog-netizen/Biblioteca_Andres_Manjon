...
// Para administradores/profesores: mostrar TODOS los préstamos
$sql_prestamos = "
    SELECT 
        p.id_prestamo,
...
";
// Elimina el bind_param y el WHERE p.id_usuario = ?
        $param = $id_logueado;
    } else {
    ...
    if ($resultado_prestamos && $resultado_prestamos->num_rows > 0) {
        ...
        if ($id_rol === 3) { // Solo para alumnos
            echo "<p><a href='peticion.php' class='btn' style='margin-top: 15px;'>Solicitar un préstamo</a></p>";
        }
        echo "</div>";
    }
